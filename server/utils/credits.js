...
export const consumeCredits = async ({ user, toolSlug, metadata }) => {
  ...
  if (user.availableCredits < tool.credits) {
    const error = new Error(
      "Insufficient credits. Upgrade or buy more credits.",
    );
  ...
  const { updatedUser, toolUsage } = await prisma.$transaction(
    async (transaction) => {
      ...
      await transaction.creditTransaction.create({
        data: {
          ...
          type: "TOOL_USAGE",
          amount: -tool.credits,
          balanceAfter: user.availableCredits - tool.credits,
          toolSlug: tool.slug,
          reason: `Used ${tool.name}`,
        },
      });
      ...
      return {
        updatedUser: await transaction.user.findUnique({
          where: { id: user.id },
        }),
...
export const refundCredits = async ({ userId, tool, usageId, reason }) => {
  ...
  await prisma.$transaction(async (transaction) => {
    if (usageId) {
      const usage = await transaction.toolUsage.findUnique({
        where: { id: usageId },
        select: { success: true },
      });
    ...
    if (usageId)
      await transaction.toolUsage.update({
        where: { id: usageId },
        data: { success: false },
      });
  });
